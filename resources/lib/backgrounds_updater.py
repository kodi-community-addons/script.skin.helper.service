...
class BackgroundsUpdater(threading.Thread):
    ...
    def run(self):
        ...
        while (self.exit != True):
            ...
            #Process backgrounds
            if xbmc.getCondVisibility("![Window.IsActive(fullscreenvideo) | Window.IsActive(script.pseudotv.TVOverlay.xml) | "\
                "Window.IsActive(script.pseudotv.live.TVOverlay.xml)] | Window.IsActive(script.pseudotv.live.EPG.xml)"):
    ...
    def get_skinconfig(self):
        ...
        self.custom_picturespath = xbmc.getInfoLabel("skin.string(SkinHelper.CustomPicturesBackgroundPath)").decode("utf-8")
    ...
    def get_images_from_path(self, lib_path):
        ...
        if "plugin.video.emby" in lib_path and "browsecontent" in lib_path and not "filter" in lib_path:
    ...
    def setPicturesBackground(self,win_prop):
        ...
        if images:
            ...
            for key, value in image.iteritems():
                if key == "fanart": self.win.setProperty(win_prop, value)
                else: self.win.setProperty(win_prop + "." + key, value)
    ...
    def set_background(self, win_prop, images, fallback_image=""):
        if images:
            if not win_prop in self.all_backgrounds_keys:
    ...
    def update_backgrounds(self):
        ...
        if xbmc.getCondVisibility("Library.HasContent(movies)"):
            ...
            self.set_background("SkinHelper.InProgressMoviesBackground",
                self.get_images_from_path("plugin://script.skin.helper.widgets?mediatype=movies&action=inprogress&limit=50"))
            self.set_background("SkinHelper.RecentMoviesBackground",
                self.get_images_from_path("videodb://recentlyaddedmovies/"))
            self.set_background("SkinHelper.UnwatchedMoviesBackground",
                self.get_images_from_path("plugin://script.skin.helper.widgets?mediatype=movies&action=unwatched&limit=50"))

        #tvshows backgrounds
        if xbmc.getCondVisibility("Library.HasContent(tvshows)"):
            self.set_background("SkinHelper.AllTvShowsBackground",
                self.get_images_from_path("videodb://tvshows/titles/"))
            self.set_background("SkinHelper.InProgressShowsBackground",
                self.get_images_from_path("plugin://script.skin.helper.widgets?mediatype=tvshows&action=inprogress&limit=50"))
        ...
        if xbmc.getCondVisibility("Library.HasContent(music)"):
            ...
            self.set_background("SkinHelper.AllMusicSongsBackground",
                self.get_images_from_path("plugin://script.skin.helper.widgets/?mediatype=songs&action=random&limit=50"))
            self.set_background("SkinHelper.RecentMusicBackground",
                self.get_images_from_path("plugin://script.skin.helper.widgets/?mediatype=albums&action=recent&limit=50"))
        ...
        if xbmc.getCondVisibility("PVR.HasTvChannels"):
            self.set_background("SkinHelper.PvrBackground", 
                self.get_images_from_path("plugin://script.skin.helper.widgets/?mediatype=pvr&action=recordings&limit=50"))
        ...
        if xbmc.getCondVisibility("![Library.HasContent(movies) | Library.HasContent(tvshows) | Library.HasContent(music)] + System.HasAddon(script.extendedinfo)"):
            self.set_global_background("SkinHelper.GlobalFanartBackground", [ "SkinHelper.TopRatedMovies", "SkinHelper.TopRatedShows" ])
        else:
            self.set_global_background("SkinHelper.GlobalFanartBackground", [ "SkinHelper.AllMoviesBackground", "SkinHelper.AllTvShowsBackground", "SkinHelper.AllMusicVideosBackground","SkinHelper.AllMusicBackground" ])
            self.set_global_background("SkinHelper.AllVideosBackground", [ "SkinHelper.AllMoviesBackground", "SkinHelper.AllTvShowsBackground", "SkinHelper.AllMusicVideosBackground" ])
            self.set_global_background("SkinHelper.AllVideosBackground2", [ "SkinHelper.AllMoviesBackground", "SkinHelper.AllTvShowsBackground" ])
            self.set_global_background("SkinHelper.RecentVideosBackground", [ "SkinHelper.RecentMoviesBackground", "SkinHelper.RecentEpisodesBackground" ])
            self.set_global_background("SkinHelper.InProgressVideosBackground", [ "SkinHelper.InProgressMoviesBackground", "SkinHelper.InProgressShowsBackground" ])
