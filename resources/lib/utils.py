...
def get_listitem_details(content_type, prefix, litype="ListItem"):
    ...
    for prop in props:
        propvalue = xbmc.getInfoLabel('%s%s.%s' % (prefix, litype, prop)).decode('utf-8')
        if not propvalue or propvalue == "-1":
            propvalue = xbmc.getInfoLabel('%s%s.Property(%s)' % (prefix, litype, prop)).decode('utf-8')
        if "art(" in prop:
