    ...
    try:
        if xbmcvfs.exists("special://skin/shortcuts/"):
            shortcuts_file = xbmc.translatePath(
                "special://home/addons/script.skinshortcuts/resources/shortcuts/info-window-home-property-%s-title.DATA.xml" 
                    %win_prop.replace(".","-")).decode("utf-8")
            templatefile = "special://home/addons/%s/resources/smartshortcuts/smartshortcuts-submenu-template.xml"%ADDON_ID
...
def get_content_path(lib_path):
    ...
    if "$INFO" in lib_path and not "reload=" in lib_path:
