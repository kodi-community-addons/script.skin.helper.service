...
def getWidgets(itemstoInclude=None):
    ...
    for widgetType in itemstoInclude:
        if widgetType == "smartshortcuts":
            widgets = getSmartShortCutsWidgetNodes()
        elif widgetType == "skinplaylists":
            widgets = getPlayListsWidgetListing()
        elif widgetType == "favourites":
            widgets = getFavouritesWidgetsListing()
        elif widgetType in ["pvr", "smartishwidgets", "static"]:
            widgets = getOtherWidgetsListing(widgetType)
        else:
            widgets = getAddonWidgetListing(widgetType)
...
def getBackgrounds():
    ...
    globalBackgrounds.append(
        ("%s - %s" %
         (ADDON.getLocalizedString(32117),
          ADDON.getLocalizedString(32161)),
            "SkinHelper.AllMoviesBackground.Wall.BW"))
    globalBackgrounds.append(
        ("%s (%s)" %
         (ADDON.getLocalizedString(32117),
          ADDON.getLocalizedString(32156)),
    ...
    globalBackgrounds.append(
        ("%s (%s) - %s" %
         (ADDON.getLocalizedString(32117),
          ADDON.getLocalizedString(32156),
          ADDON.getLocalizedString(32161)),
            "SkinHelper.AllMoviesBackground.Poster.Wall.BW"))
    ...
    globalBackgrounds.append(
        ("%s - %s" %
         (ADDON.getLocalizedString(32127),
          ADDON.getLocalizedString(32161)),
            "SkinHelper.AllMusicBackground.Wall.BW"))
    ...
    globalBackgrounds.append(
        ("%s - %s" %
         (ADDON.getLocalizedString(32119),
          ADDON.getLocalizedString(32161)),
            "SkinHelper.AllMusicSongsBackground.Wall.BW"))
    ...
    globalBackgrounds.append(
        ("%s - %s" %
         (ADDON.getLocalizedString(32127),
          ADDON.getLocalizedString(32161)),
            "SkinHelper.AllTvShowsBackground.Wall.BW"))
    globalBackgrounds.append(
        ("%s (%s)" %
         (ADDON.getLocalizedString(32127),
          ADDON.getLocalizedString(32156)),
    ...
    globalBackgrounds.append(
        ("%s (%s) - %s" %
         (ADDON.getLocalizedString(32127),
          ADDON.getLocalizedString(32156),
          ADDON.getLocalizedString(32161)),
            "SkinHelper.AllTvShowsBackground.Poster.Wall.BW"))

    if xbmc.getCondVisibility("System.HasAddon(script.extendedinfo)"):
        globalBackgrounds.append(
            (xbmc.getInfoLabel("$ADDON[script.extendedinfo 32046]") +
             " (TheMovieDB)",
             "SkinHelper.TopRatedMovies"))
        globalBackgrounds.append(
            (xbmc.getInfoLabel("$ADDON[script.extendedinfo 32040]") +
             " (TheMovieDB)",
            ...
            for item in media_array:
                if item["file"].endswith(".xsp"):
                    ...
                    try:
                        ...
                        pass
                    if not mediaType:
                        mediaType = detect_plugin_content(playlist)
...
def buildAddonWidgetItem(pluginpath, sublevel=""):
    ...
    for item in media_array:
        ...
        if ("script.extendedinfo" in pluginpath and hasTMDBCredentials == False and (
...
def getFavouritesWidgetsListing():
    ...
    json_result = kodi_json('Favourites.GetFavourites',
                            '{"type": null, "properties": ["path", "thumbnail", "window", "windowparameter"]}')
    foundWidgets = []
...
def getOtherWidgetsListing(widget):
    ...
    if widget == "pvr" and xbmc.getCondVisibility("PVR.HasTVChannels"):
        foundWidgets.append(["$ADDON[script.skin.helper.service 32153]",
                             "pvr://channels/tv/;reload=$INFO[Window(Home).Property(widgetreload2)]", "pvr"])
        foundWidgets.append(
            [
                "$ADDON[script.skin.helper.service 32170]",
                "plugin://script.skin.helper.service/?action=pvrchannels&limit=25&reload=$INFO[Window(home).Property(widgetreload2)]",
                "pvr"])
        foundWidgets.append(
            [
                "$ADDON[script.skin.helper.service 32151]",
                "plugin://script.skin.helper.service/?action=pvrrecordings&limit=25&reload=$INFO[Window(home).Property(widgetreload2)]",
                "pvr"])
        foundWidgets.append(
            [
                "$ADDON[script.skin.helper.service 32152]",
                "plugin://script.skin.helper.service/?action=nextpvrrecordings&limit=25&reload=$INFO[Window(home).Property(widgetreload2)]",
                "pvr"])
        foundWidgets.append(
            [
                "$ADDON[script.skin.helper.service 32171]",
                "plugin://script.skin.helper.service/?action=nextpvrrecordings&reversed=true&limit=25&reload=$INFO[Window(home).Property(widgetreload2)]",
                "pvr"])
        foundWidgets.append(
            [
                "$ADDON[script.skin.helper.service 32154]",
                "plugin://script.skin.helper.service/?action=pvrtimers&limit=25&reload=$INFO[Window(home).Property(widgetreload2)]",
                "pvr"])
