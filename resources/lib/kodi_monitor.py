...
class KodiMonitor(xbmc.Monitor):
    ...
    def reset_music_widgets(self,data,resetAll=False):
        ...
        if (media_type in ["song","artist","album"] or resetAll and not self.win.getProperty("SkinHelperShutdownRequested")):
            #artutils.updateMusicArt(media_type,data.get("id"))
            if not xbmc.getCondVisibility("Library.IsScanningMusic"):
                log_msg("Music database changed - type: %s - resetAll: %s, refreshing widgets...." %(media_type,resetAll))
    ...
    def reset_video_widgets(self,data="",resetAll=False):
        ...
        if data:
            ...
            if data and data.get("item"):
                media_type = data["item"].get("type","unknown")
                id = data["item"].get("id",None)

        if (media_type in ["movie","tvshow","episode"] and not self.win.getProperty("skinhelper-refreshvideowidgetsbusy")) or resetAll:
            log_msg("Video database changed - type: %s - resetAll: %s, refreshing widgets...." %(media_type,resetAll))
            self.win.setProperty("skinhelper-refreshvideowidgetsbusy","busy")
            timestr = time.strftime("%Y%m%d%H%M%S", time.gmtime())
            #reset specific widgets, based on item that is updated
            if resetAll or media_type=="movie":
                self.win.setProperty("widgetreload-movies", timestr)
            if resetAll or media_type=="episode":
                self.win.setProperty("widgetreload-episodes", timestr)
            if resetAll or media_type=="tvshow":
            ...
            if id:
                ...
                self.artutils.get_streamdetails(db_id=id,media_type=media_type,ignore_cache=True)
